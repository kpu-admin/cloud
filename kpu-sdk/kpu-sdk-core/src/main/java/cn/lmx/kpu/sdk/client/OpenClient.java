package cn.lmx.kpu.sdk.client;

import cn.lmx.kpu.sdk.common.*;
import cn.lmx.kpu.sdk.exception.SdkException;
import cn.lmx.kpu.sdk.param.BaseParam;
import cn.lmx.kpu.sdk.param.DownloadAware;
import cn.lmx.kpu.sdk.param.DownloadParam;
import cn.lmx.kpu.sdk.sign.SignUtil;
import cn.lmx.kpu.sdk.sign.SopSignException;
...
public class OpenClient {
     ...
     *
     * @param request 请求对象
     * @param <Resp>     返回对应的Response
     * @return 返回Response
     */
    public <Req, Resp> Result<Resp> execute(BaseParam<Req, Resp> request) {
        return this.execute(request, null);
    }

    public FileResult download(DownloadParam request) {
        return download(request, null);
    }

    public FileResult download(DownloadParam request, String accessToken) {
     ...
     * @return 返回Response
     */
    public <Req, Resp> Result<Resp> execute(BaseParam<Req, Resp> request, String accessToken) {
     ...
     */
    public <Req, Resp> Result<Resp> parseResponse(String resp, BaseParam<Req, Resp> request) {
        String method = request.getMethod();
        String rootNodeName = dataNameBuilder.build(method);
        JSONObject jsonObject = JSON.parseObject(resp, JSONReader.Feature.SupportAutoType);
        ...
        if (data != null) {
            Resp dataObj = data.toJavaObject(request.getResponseClass());
